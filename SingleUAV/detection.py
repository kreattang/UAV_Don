...
while True:
    ...
    if track_ok:
        ...
        # cv2.putText(frame,"Center:"+str(int(bbox[0] + bbox[2] / 2))+ str(int(bbox[1] + bbox[3] / 2)),cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 4)
        print("中心：",int(bbox[0] + bbox[2] / 2), int(bbox[1] + bbox[3] / 2))
        ...
        cv2.putText(frame, "No tracking target selected", (100, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
    elif not track_ok:
        cv2.putText(frame, "Tracking failure detected", (100, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
    ...
    # 显示结果
    cv2.imshow("ZSTU UAV Tracker", frame)
    # 按ESC退出
